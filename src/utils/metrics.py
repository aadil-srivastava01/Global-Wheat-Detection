...
def create_anchors(sizes, ratios, scales, flatten=True):
    ...
    aspects = [[[s*math.sqrt(r), s*math.sqrt(1/r)]
...
class RetinaNetFocalLoss(nn.Module):
    def __init__(self, model, gamma: float = 2., alpha: float = 0.25,  pad_idx: int = 0, scales=None, ratios=None, reg_loss=Fun.smooth_l1_loss):
        super().__init__()
        self.model = model
    ...
    def _create_anchors(self, sizes, device: torch.device):
        ...
        self.anchors = create_anchors(
            sizes, self.ratios, self.scales).to(device)
    ...
    def _focal_loss(self, clas_pred, clas_tgt):
        ...
        clas_loss = Fun.binary_cross_entropy_with_logits(
